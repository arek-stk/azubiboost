/**
 * Lernpfad fürs Rechnen: erst leicht, dann schwerer.
 *
 * Die Aufgabentypen sind in vier Stufen geordnet. Ein Typ gilt als gemeistert,
 * wenn er oft genug ohne Hilfe richtig gelöst wurde. Aufgaben, die sie mit Hilfe
 * durchgerechnet hat, zählen nicht, damit „gemeistert" heißt: kann ich allein.
 * Gesperrt wird nichts. Der Pfad gibt eine Empfehlung, sie darf aber jede
 * Aufgabe öffnen.
 */

import type { AufgabentypId } from './rechenaufgaben'

export type Stufe = { nr: 1 | 2 | 3 | 4; name: string; beschreibung: string; typen: readonly AufgabentypId[] }

export const STUFEN: readonly Stufe[] = [
  {
    nr: 1,
    name: 'Grundlagen',
    beschreibung: 'Rechnen wie an der Kasse, dann Prozent, Rabatt, Steuer und Runden. Alles mit einfachen Zahlen.',
    typen: [
      'g-menge-preis',
      'g-wechselgeld',
      'g-durchschnitt',
      'g-prozentwert',
      'g-prozentsatz',
      'g-grundwert',
      'g-aufschlag',
      'g-abzug',
      'g-herausrechnen',
      'g-runden',
    ],
  },
  {
    nr: 2,
    name: 'Einstieg',
    beschreibung: 'Die ersten Prüfungsaufgaben. Sie sind kurz und haben wenige Schritte.',
    typen: [
      'dreisatz',
      'kassenabrechnung',
      'umsatzsteuer',
      'grundpreis',
      'flaechenkennzahlen',
      'meldebestand',
      'prozentveraenderung',
      'bezugspreis',
      'verkaufspreis',
      'rohgewinn',
    ],
  },
  {
    nr: 3,
    name: 'Sicher werden',
    beschreibung: 'Mehr Schritte und neue Themen wie Handelsspanne, Lagerkennzahlen, Steuer und Rentabilität.',
    typen: [
      'differenz',
      'handelsspanne',
      'kalkulationsfaktor',
      'rueckwaerts',
      'inventurdifferenz',
      'lagerkennzahlen',
      'handlungskostensatz',
      'zahllast',
      'angebotsvergleich',
      'break-even',
      'rentabilitaet',
    ],
  },
  {
    nr: 4,
    name: 'Prüfungsniveau',
    beschreibung: 'Lange Rechnungen mit vielen Schritten, so wie sie in der Prüfung vorkommen.',
    typen: [
      'verkaufspreis-komplett',
      'rueckwaerts-komplett',
      'differenz-komplett',
      'lagerkennzahlen-jahr',
      'lagerzinsen',
      'skontovergleich',
    ],
  },
]

export function meisterSchwelle(id: AufgabentypId): number {
  return id.startsWith('g-') ? 2 : 3
}

export type KnotenStand = {
  typId: AufgabentypId
  stufe: Stufe['nr']
  richtig: number
  schwelle: number
  gemeistert: boolean
  jetztDran: boolean
}

export type PfadStand = {
  knoten: KnotenStand[]
  /** Der nächste nicht gemeisterte Typ in Pfadreihenfolge; null, wenn alles sitzt. */
  naechster: AufgabentypId | null
  aktuelleStufe: Stufe['nr']
  gemeistert: number
  gesamt: number
}

export function lernpfad(richtigJeTyp: Partial<Record<AufgabentypId, { richtig: number }>>): PfadStand {
  const roh = STUFEN.flatMap((s) =>
    s.typen.map((typId) => {
      const richtig = richtigJeTyp[typId]?.richtig ?? 0
      const schwelle = meisterSchwelle(typId)
      return { typId, stufe: s.nr, richtig, schwelle, gemeistert: richtig >= schwelle }
    }),
  )
  const naechster = roh.find((k) => !k.gemeistert)?.typId ?? null
  const knoten = roh.map((k) => ({ ...k, jetztDran: k.typId === naechster }))
  const aktuelleStufe = knoten.find((k) => k.jetztDran)?.stufe ?? 4
  return {
    knoten,
    naechster,
    aktuelleStufe,
    gemeistert: knoten.filter((k) => k.gemeistert).length,
    gesamt: knoten.length,
  }
}
